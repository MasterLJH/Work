...
namespace BingLibrary.hjb.DataBase
{
    public abstract class DbHelperOleDb
    {
        //数据库连接字符串，可以动态更改connectionString支持多数据库.
        public static string connectionString = "Provider=MSDAORA.1;User ID=fesl002;Password=feslfesl;Data Source=(DESCRIPTION = (ADDRESS_LIST= (ADDRESS = (PROTOCOL = TCP)(HOST = 10.16.137.59)(PORT = 1521))) (CONNECT_DATA = (SERVICE_NAME = FESLMES)))";
        ...
        public static void ReadConnectionString()
        {
            try
            {
                connectionString = File.ReadAllText("OracleConnectionString.ini");
            }
            catch (Exception ex)
            {
